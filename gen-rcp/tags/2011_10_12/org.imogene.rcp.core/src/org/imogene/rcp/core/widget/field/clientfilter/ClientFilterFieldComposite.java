...
/**
 * Widget that enables to create a form field for the client filter view
 * @author MEDES-IMPS
 */
public abstract class ClientFilterFieldComposite extends Composite {
	 ...
	 */
	public ClientFilterFieldComposite(Composite parent, FormToolkit toolkit, String fieldLabelText, boolean dependent){
	 ...
	 * @param parent
	 */
	public ClientFilterFieldComposite(Composite parent) {
		super(parent, SWT.NONE);		
	}
	 ...
	 * @param isRequired
	 */
	protected void createComposite(FormToolkit toolkit, String fieldLabelText) {
		createColors(toolkit);
		composeComposite(toolkit, fieldLabelText);
	}
	...
	private void composeComposite(FormToolkit toolkit, String fieldLabelText) {
		...
		GridData labelLayoutData = new GridData(GridData.HORIZONTAL_ALIGN_END | GridData.VERTICAL_ALIGN_CENTER);
		...
		// add field operator
		createFieldOperator(toolkit);
		fillOperatorCombo();
		fieldOperator.setLayoutData(new GridData(GridData.GRAB_HORIZONTAL | GridData.HORIZONTAL_ALIGN_BEGINNING | GridData.FILL_HORIZONTAL | GridData.VERTICAL_ALIGN_CENTER ));
...
//	}	
	 ...
	 * @param toolkit
	 */
	protected void createFieldOperator(FormToolkit toolkit) {
		fieldOperator = new Combo(this, SWT.SINGLE|SWT.READ_ONLY);
	}
	
	/**
	 * Fills the operator combobox with operator values
	 */
	protected abstract void fillOperatorCombo() ;	
	 ...
	 */
	protected void addOperator(String display, String value){
		fieldOperator.add(display);
		fieldOperator.setData(String.valueOf(fieldOperator.getItemCount()-1), value);
	}
	 ...
	 */
	public void setOperator(String result){
		for(int i=0; i<fieldOperator.getItemCount();i++){
			String data = (String)fieldOperator.getData(String.valueOf(i));
			if(data.equals(result)){
				fieldOperator.select(i);
				break;
			}
		}
	}
	...
	public String getOperator(){
		return (String)fieldOperator.getData(String.valueOf(fieldOperator.getSelectionIndex()));
	}
