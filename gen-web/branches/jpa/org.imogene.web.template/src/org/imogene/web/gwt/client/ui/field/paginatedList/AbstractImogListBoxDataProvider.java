...
/**
 * Data provider for the list box that displays a big quantity of data
 * 
 * @author MEDES-IMPS
 */
public abstract class AbstractImogListBoxDataProvider implements
		ImogPaginatedListBoxDataProvider {
	 ...
	 * ImogenePaginatedListBoxDataProvider#updateRowData(int, int,
	 * org.imogene.web
	 * .gwt.client.ui.field.paginatedList.ImogenePaginatedListBoxDataProvider
	 * .RowDataAcceptor)
	 */
	public void updateRowData(int startRow, int maxRows,
	...
	/**
	 * Asynchronous call to the server. Gets the data that will fill the
	 * MedanyPaginatedList.
	 */
	protected abstract void getData(int startRow, int maxRows);

	/**
	 * Asynchronous call to the server. Counts the total number of rows of the
	...
	private void pushResults(RowDataAcceptor acceptor, int startRow,
			ImogBean[] array, int totalNbOfRows) {
	 ...
	 * 
	 * @param isFiltered
	 *            true if the dataprovider is permanently filtered
	 */
	 ...
	 */
	/*
	 * public class PushResultCallback implements AsyncCallback<List<? extends
	 * ImogBean>> {
	 * 
	 * public void onFailure(Throwable caught) { acceptor.failed(caught); }
	 * 
	 * public void onSuccess(List<? extends ImogBean> result) {
	 * 
	 * ImogBean[] beanArray = new ImogBean[result.size()]; for (int i = 0; i <
	 * beanArray.length; i++) { beanArray[i] = result.get(i); } lastBean =
	 * beanArray;
	 * 
	 * get total nb of rows from service countDataRows(); }
	 * 
	 * }
	 */
