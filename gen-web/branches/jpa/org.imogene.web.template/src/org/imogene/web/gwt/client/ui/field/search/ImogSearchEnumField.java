...
public class ImogSearchEnumField extends ImogSearchFieldAbstract<String> {
	...
	/**
	 * @param multiple
	 *            true if multiple selection are allowed
	 */
	 ...
	 *            true if multiple selection are allowed
	 * @param list
	 *            true if single selection are presented as a list.
	 */
	 ...
	 *            true if multiple selection are allowed
	 * @param list
	 *            true if single enumeration have to be displayed in a list box.
	 * @param label
	 *            The field label
	 */
	...
	private void layout() {
		...
		if (!listWidget) {
			...
			layout.setCellVerticalAlignment(fieldPanel,
					HasVerticalAlignment.ALIGN_MIDDLE);
			layout.setCellHorizontalAlignment(fieldPanel,
			...
			layout.setCellVerticalAlignment(listChoices,
					HasVerticalAlignment.ALIGN_MIDDLE);
			layout.setCellHorizontalAlignment(listChoices,
					HasHorizontalAlignment.ALIGN_LEFT);
		}
	}
	...
	public boolean toBeSearched() {
		return (getOperatorValue() != null && getOperatorValue().equals(
				CriteriaConstants.OPERATOR_ISNULL_OR_EMPTY))
				|| (getOperatorValue() != null && getValue() != null && !getValue()
						.equals(""));
	}
	...
	protected void fillOperatorValues() {
		...
		if (thisMultiple) {
			thisOperator.addItem(BaseNLS.constants()
					.search_operator_contains_all(),
					CriteriaConstants.ENUM_MULT_OPERATOR_CONTAINS_ALL);
			thisOperator.addItem(BaseNLS.constants()
					.search_operator_contains_one(),
					CriteriaConstants.ENUM_MULT_OPERATOR_CONTAINS_ONE_OF);
		} else
			thisOperator.addItem(BaseNLS.constants()
					.search_operator_string_equal(),
					CriteriaConstants.STRING_OPERATOR_EQUAL);
	...
	/**
	 * Embedded panel that display each selectable item.
	 */
	private class EnumFieldPanelList extends VerticalPanel {
		...
		/**
		 * Get the string value representation of the selected widgets
		 * 
