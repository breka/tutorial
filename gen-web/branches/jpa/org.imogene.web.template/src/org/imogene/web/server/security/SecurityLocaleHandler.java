...
public class SecurityLocaleHandler extends
	...
	public void onAuthenticationSuccess(HttpServletRequest request,
		...
		if (savedRequest != null) {
			...
			if (modifyReditectUrl(request, redirectUrl)) {
				logger.debug("SaveRequest is not null and we modified the url: "
						+ modifiedUrl);
	 ...
	 * @param redirectUrl
	 *            the saved redirect URL
	 * @return true if the URL have been modified
	 * @throws MalformedURLException
	 */
	private boolean modifyReditectUrl(HttpServletRequest request,
			String redirectUrl) throws MalformedURLException {
		...
		/*
		 * if it is a request to a RPC service we redirect to the entry point
		 * page
		 */
		...
		/*
		 * if a 'locale' parameter is present in the login page request, we
		 * report it to the saved request URL
		 */
		if (request.getParameter(LOCALE_PARAM) != null) {
			URLBuilder builder = new URLBuilder(url);
			if (builder.getFirstValueParameter(LOCALE_PARAM) == null) {
				if (request.getParameter(LOCALE_PARAM) != null) {
					builder.setParameter(LOCALE_PARAM,
							request.getParameterValues(LOCALE_PARAM));
