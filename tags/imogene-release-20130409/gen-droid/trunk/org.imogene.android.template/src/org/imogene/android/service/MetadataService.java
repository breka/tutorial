...
public class MetadataService extends WakefulIntentService {
	...
	public static final void startMetadataService(Context context,
	...
	public static final void startMetadataService(Context context, Exception e,
			long time) {
		Intent i = new Intent();
		i.setClass(context, MetadataService.class);
	...
	protected void doWakefulWork(Intent intent) {
		try {
			...
			File file = new File(Paths.PATH_SYNCHRO, System.currentTimeMillis()
					+ ".metadata");
			...
			if (intent.hasExtra(EXTRA_VALUES)) {
				...
				for (Entry<String, Object> value : values.valueSet()) {
					if (value.getValue() != null) {
						...
						serializer.attribute(null, ATTR_VALUE, value.getValue()
								.toString());
				...
				Exception e = (Exception) intent
						.getSerializableExtra(EXTRA_EXCEPTION);
	...
	public static class MetadataClient {
		...
		public final int sendMetaData(FileInputStream fis) throws Exception {
			try {
				...
				StringBuilder builder = new StringBuilder(mServer).append(
						"?cmd=meta").append("&id=" + uuid);

				HttpPost method = new HttpPost(builder.toString());

				String fileName = uuid + ".metadata";
				FileInputStreamPart part = new FileInputStreamPart("data",
						fileName, fis);
