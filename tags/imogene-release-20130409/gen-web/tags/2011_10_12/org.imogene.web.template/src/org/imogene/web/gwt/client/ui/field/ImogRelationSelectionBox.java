...
public abstract class ImogRelationSelectionBox<T extends ImogBean> extends PopupPanel implements ClickHandler {
		...
		if(event.getSource().equals(ok)){
			for(int i=0; i<listSyncs.getItemCount(); i++){
				if(listSyncs.isItemSelected(i)){
					parentField.addValue(listSyncs.getItemText(i), alls.get(listSyncs.getValue(i)));					
				}
			}
			hide();
		}
