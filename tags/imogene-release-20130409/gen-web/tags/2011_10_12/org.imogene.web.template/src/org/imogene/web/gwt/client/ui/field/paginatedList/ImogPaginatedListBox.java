...
/**
 * This class enables to display a listbox that manages a large quantity of data through paginated lists.
 * Data are retrieved by an asynchronous call, and paginated in the list box.
 * @author MEDES-IMPS
 */
public class ImogPaginatedListBox<T extends ImogBean> extends Composite implements ClickHandler {
	...
	private void layout() {
		...
		panel.add(textBox);
		panel.setCellHorizontalAlignment(textBox, HasHorizontalAlignment.ALIGN_LEFT);
		...
		panel.add(image);
		panel.setCellHorizontalAlignment(image, HasHorizontalAlignment.ALIGN_LEFT);
	 ...
	 * Sets the listbox selected value 
	 * @param value the selected value 
	 * @param notifyChange if true, notifies the value setup to the added handlers
	 */
	 ...
	 * @param enable true if this widget is enable
	 */
	 ...
	 * @param mainFieldsUtil the utility class to get a display value of the list bean instances
	 */
	public void setDataProvider(AbstractImogListBoxDataProvider provider, MainFieldsUtil mainFieldsUtil){
		this.provider = provider;
		this.mainFieldsUtil = mainFieldsUtil;
	}
	
	/**
	 * Explicitly adds a bean instance to the listbox.
	 * The list box will not be paginated
	 * @param display
	 * @param id
	 * @param value
	 */
	...
	public void onClick(ClickEvent event) {	
		if(isEnabled && (event.getSource().equals(image) || event.getSource().equals(textBox))){
			if(isPaginated) {
				popupList = new ImogPaginatedListBoxPopup<T>(provider, mainFieldsUtil, this);
			}
			else {
				if (popupList==null || popupList.isPaginated)
					popupList = new ImogPaginatedListBoxPopup<T>(this);				
			}	
			popupList.setPopupPosition(getAbsoluteLeft(), getAbsoluteTop()+getOffsetHeight()-1);			
			popupList.show();		
		}
	}
