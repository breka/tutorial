...
/**
 * Interface that describes a data provider for the list box
...
public interface ImogPaginatedListBoxDataProvider {
	 ...
	 * @param maxRows number max of row to retrieve
	 * @param acceptor The acceptor that have in charge to populate the widget
	 */
	...
	/**
	 * Interface that describes the class that has to populate 
	 * the list box with the received data.
	 * @author MEDES-IMPS
	 *
	 */
	interface RowDataAcceptor {
		 ...
		 * @param rowsDisplay Array of the values to display
		 * @param rowsValue Array of the values associated with the display value.
		 */
	    void accept(int startRow, ImogBean[] array, int totalNbOfRows);
	    
	    /**
	     * Called when an error occurred retrieving the data.
	     * @param caught the error caught
	     */
