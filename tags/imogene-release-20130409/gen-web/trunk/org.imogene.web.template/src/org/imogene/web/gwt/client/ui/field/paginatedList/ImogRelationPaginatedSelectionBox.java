...
import org.imogene.web.gwt.client.ui.field.ImogRelationList;
...
public class ImogRelationPaginatedSelectionBox<T extends ImogBean> extends PopupPanel implements ClickHandler {
	...
	//private HashSet<AssociationSelectionListener> listeners = new HashSet<AssociationSelectionListener>();

	
	public ImogRelationPaginatedSelectionBox(String colorStyle, AbstractImogListBoxDataProvider provider, MainFieldsUtil mainFieldsUtil){
		super(true);	
			...
			if (pageList.getValues() != null) {
				...
				for(ImogBean value:values) {
					...
					if(!parentField.isPresent(result))
						parentField.addValue(mainFieldsUtil.getDisplayValue(result), result);
				}
			}						
			hide();
		}
