            ...
            for (int i=0; i<msgs.length; i++){
            	...
            	if (KEY.equals(body[0])) {
            		...
            		if (Long.parseLong(body[1]) == smsnb) {
            			PreferenceHelper.getSharedPreferences(context).edit().putLong(context.getString(W.string.debug_smsnb_key), ++smsnb).commit();
            			if (body[2].equals("r")) {
	...
	private void performRead(Context context, String[] body) {
		...
		for (int i = 3; i < body.length; i++) {
			...
			case READ_ENTNB:
				...
				break;
			case READ_SYNCSTAT:
				String syncStatKey = context.getString(W.string.sync_on_off_key);
				values.put(syncStatKey, PreferenceHelper.getSynchronizationStatus(context));
				break;
			case READ_SYNCURL:
				String serverUrlKey = context.getString(W.string.sync_server_url_key);
				values.put(serverUrlKey, PreferenceHelper.getServerUrl(context));
				break;
			case READ_SYNCPER:
				String periodeKey = context.getString(W.string.sync_time_key);
				values.put(periodeKey, PreferenceHelper.getSynchronizationPeriod(context));
				break;
			case READ_OFFSET:
				String offsetKey = context.getString(W.string.ntp_offset_key);
				values.put(offsetKey, PreferenceHelper.getNtpOffset(context));
				break;
			case READ_NTP:
				String ntpServerKey = context.getString(W.string.ntp_server_key);
				values.put(ntpServerKey, PreferenceHelper.getNtpServerUrl(context));
				break;
			case READ_DEBUG:
				String debugKey = context.getString(W.string.debug_save_key);
	...
	private void performWrite(Context context, String[] body) {
		for (int i = 3; i < body.length; i++) {
			...
			case WRITE_FORCE_SYNCHRO:
				PreferenceHelper.getSharedPreferences(context).edit().putBoolean(context.getString(W.string.sync_on_off_key), true).commit();
				context.sendBroadcast(new Intent(Intents.ACTION_RESCHEDULE));
				break;
			case WRITE_STOP_SYNCHRO:
				PreferenceHelper.getSharedPreferences(context).edit().putBoolean(context.getString(W.string.sync_on_off_key), false).commit();
				context.sendBroadcast(new Intent(Intents.ACTION_CANCEL));
				break;
			case WRITE_ACTIVATE_DEBUG:
				PreferenceHelper.getSharedPreferences(context).edit().putBoolean(context.getString(W.string.debug_save_key), true).commit();
				break;
			case WRITE_DEACTIVATE_DEBUG:
				PreferenceHelper.getSharedPreferences(context).edit().putBoolean(context.getString(W.string.debug_save_key), false).commit();
				break;
			case WRITE_SYNCURL:
				PreferenceHelper.getSharedPreferences(context).edit().putString(context.getString(W.string.sync_server_url_key), body[++i]).commit();
				break;
			case WRITE_SYNCPER:
				PreferenceHelper.getSharedPreferences(context).edit().putString(context.getString(W.string.sync_time_key), body[++i]).commit();
				break;
			case WRITE_CLEAR_USER:
				PreferenceHelper.getSharedPreferences(context).edit()
				.remove(context.getString(W.string.current_login_key))
				.remove(context.getString(W.string.current_roles_key))
				.remove(context.getString(W.string.sync_login_key))
				.remove(context.getString(W.string.sync_password_key))
				.remove(context.getString(W.string.sync_roles_key))
				.remove(context.getString(W.string.sync_shortpw_key))
				.remove(context.getString(W.string.sync_server_url_key))
				.commit();
				break;
			default:
				break;
