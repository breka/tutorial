...
/**
 * Fields that presents an address in a text area and proposes a button, to
 * throw an action on click. The default action open the point corresponding to
 * the address on the GoogleMaps web site.
 * 
 * @author Medes-IMPS
 */
public class ImogGeoField extends ImogFieldAbstract<String> implements
		ClickHandler {
	 ...
	 * 
	 * @param behavior
	 *            the behavior when the field is clicked
	 */
	...
	private void layout() {
		...
		viewOnMap = new Image(GWT.getModuleBaseURL()
				+ "/images/view_on_map-16.png");
		main.add(textArea);
		main.add(viewOnMap);
		initWidget(main);
		properties();
	...
	/**
	 * Interface which describes the behavior of teh widget when the user choose
	 * to open thes address.
	 */
	...
	/**
	 * Behavior that permits to display the address location on the Google maps
	 * web site.
	 * 
	 * @author Medes-IMPS
	 */
	...
	public static class DefaultAddressBehavior implements AddressFieldBehavior {
		...
		public void addressAction(String point) {
			if (point != null) {
				...
				if (ll.length > 1) {
					Window.open(GMAPS_TMPL.replace("%LONGITUDE%", ll[0])
							.replace("%LATITUDE%", ll[1]), "Google maps", "");
				}
			}
		}
	}
