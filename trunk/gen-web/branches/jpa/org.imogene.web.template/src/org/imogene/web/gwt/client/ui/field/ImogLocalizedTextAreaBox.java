...
import org.imogene.common.localized.LocalizedText;
 ...
 * @author Medes-IMPS
 */
public class ImogLocalizedTextAreaBox extends ImogFieldAbstract<String>
	...
	private void properties() {
		...
		layout.setCellVerticalAlignment(localeListBox,
				HasVerticalAlignment.ALIGN_MIDDLE);
		// layout.setCellHorizontalAlignment(localeListBox,
		// HasHorizontalAlignment.ALIGN_LEFT);
	}

	/**
	 * Creates the listbox that lists the availables locales for the Localized
	 * TextBox
	 * 
	 * @param isoCodes
	 *            list of iso codes
	...
	/**
	 * Sets the LocalizedTextBox value (the field id) and retrieves all the
	 * localized texts for the given field
	 * 
	 * @param value
	 *            the field id
	 */
	...
	public void setValue(List<LocalizedText> texts) {
		...
		if (texts != null && texts.size() > 0) {
			...
			// if no text is available for the current locale, get the first one
			// that is available
	...
	private boolean hasText() {
		...
		if (textArea.getText() != null && !textArea.getText().equals("")) {
			...
			for (LocalizedText text : values.values()) {
				if (!text.getLocale().equals(currentLocale)
						&& text.getValue() != null
	...
	/**
	 * Gets the localized texts to be saved defined for the LocalizedTextBox
	 */
	...
	private LocalizedText updateLocalizedText(LocalizedText text) {
		...
		if (text.getId() == null) {
			...
			text.setCreatedBy((LocalSession.get().getCurrentUser().getLogin()));
		}

		text.setModified(new Date(System.currentTimeMillis()));
		text.setModifiedBy((LocalSession.get().getCurrentUser().getLogin()));
	...
	/**
	 * Stores the text that has been entered in the TextBox in the LocalizedText
	 * map
	 */
	...
	/**
	 * Called when a new locale is selected in the locale ListBox
	 */
	@Override
	public void onChange(ChangeEvent arg0) {
	...
	public boolean validate() {
		...
		if (!textArea.getText().equals("")) {
			if (!validateRules(textArea.getText()))
				return false;
		}
