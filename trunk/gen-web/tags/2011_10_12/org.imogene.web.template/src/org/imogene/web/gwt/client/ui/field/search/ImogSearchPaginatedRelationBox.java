 ...
 * @author Medes-IMPS
 */
public class ImogSearchPaginatedRelationBox<T extends ImogBean> extends ImogSearchFieldAbstract<T> implements ListValueChangeHandler {
	...
	protected void fillOperatorValues() {
		...
		thisOperator.addItem(BaseNLS.constants().search_operator_string_equal(), CriteriaConstants.RELATIONFIELD_OPERATOR_EQUAL);
		thisOperator.addItem(BaseNLS.constants().search_operator_isnull(), CriteriaConstants.RELATIONFIELD_OPERATOR_EQUAL_NULL);
	...
	public boolean toBeSearched() {
		return (getOperatorValue() != null && getOperatorValue().equals(CriteriaConstants.RELATIONFIELD_OPERATOR_EQUAL_NULL)) || 
