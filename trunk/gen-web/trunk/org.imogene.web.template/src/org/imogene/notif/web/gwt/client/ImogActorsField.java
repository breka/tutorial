...
public class ImogActorsField extends ImogFieldAbstract<String> implements ClickHandler {
	...
	private class ActorsSelectionBox extends PopupPanel implements ClickHandler {
		...
		public void properties(){
			...
			listActors.setStylePrimaryName("imogene-RelationSelectionBox-Listbox");
			...
			if(event.getSource().equals(ok)){
				for(int i=0; i<listActors.getItemCount(); i++){
					if(listActors.isItemSelected(i)){
						list.addItem(listActors.getItemText(i), listActors.getValue(i));
					}
				}
				hide();
			}
		...
		public void show(int left, int top){			
			...
			show();
			RoleActorServiceFacade.getInstance().listActor(new AsyncCallback<List<ImogActor>>() {
				@Override
				...
				public void onSuccess(List<ImogActor> result) {				
					for(ImogActor a : result){					
						if(!isPresent(a))
						listActors.addItem(a.getLogin()+" ("+a.getId()+")", a.getId());
					}
				}				
			});
		}
